...
Five clean CSVs + a full Streamlit app, ready to deploy.

---

## 🚀 Deploy the App in 5 Minutes
...
├── README.md                    # This file
├── us_prices.csv                # US CPI data (148 rows)
├── bangladesh_wages.csv         # BD wage data (36 rows)
├── country_wages.csv            # 10-country wage comparison
├── tshirt_breakdown.csv         # T-shirt cost structure
└── combined_divergence.csv      # Hero chart data
```

### Deployment steps
1. Create a **public** GitHub repo (Streamlit Cloud needs public)
2. Upload everything in this folder to the repo root
3. Go to **https://share.streamlit.io** → sign in with GitHub
4. Click **New app** → point at your repo → main file = `app.py` → Deploy
5. First deploy takes ~2 minutes. Your app gets a URL like `yourname-fastfashion.streamlit.app`
...
### What the app does
- **6 interactive tabs:** The Divergence, Wage Freeze, Category Comparison, Country Gap, T-Shirt Calculator, ML Playground
- **Sidebar filters:** year range slider, region multi-select
- **Live K-means playground:** slide k from 2 to 6, watch clusters change, see silhouette scores
- **T-Shirt Calculator:** interactive slider showing what happens to retail price if worker wages go up 2x, 5x, 10x — a genuinely memorable demo
